...
const IvList = () => {
  ...
  const downloadFile = useMutation({
    mutationFn: async (id: string) => {
      const res = await axios.get(`/backend/smu/iv/${id}/download`, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
    ...
    onSuccess: (data) => {
      showTypedToast(EToastTypes.SUCCESS, 'Successfully downloaded IV Experiment');
      const url = window.URL.createObjectURL(new Blob([data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `iv_experiment_download_${new Date().toLocaleString('en-AU')}.csv`);
